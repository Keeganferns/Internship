...
function GoaNiwasRoomSelector({ rooms = [], selected = [], onSelect, hotelId }) {
  ...
  const occupiedCount = floorRooms.filter(r => r.status !== 'available').length;
  const totalCount = floorRooms.length;

  // Debug: Log room statuses
  console.log('Room statuses:', floorRooms.map(r => ({ id: r.id, status: r.status })));

  const handleRoomClick = (room) => {
    if (room.status !== 'available') return;
    
    // Navigate to room details in the same tab
    const roomDetailsUrl = `/hotel/${hotelId || 'niwas'}/room/${room.id}`;
...
export default GoaNiwasRoomSelector; 